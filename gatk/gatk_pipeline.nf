...
params.tmp_dir        = "/scratch/pawsey0149/mrichardson/ng_all/gatk_tmp" // Added central temp directory
...
script:
    ...
    // Memory for standalone tools (FastqToSam and MarkDuplicates)
    def mem_single = avail_mem - 8
    if (mem_single < 8) mem_single = 8
    
    // HARDCAP: Force streaming tools in the pipe to 4GB to leave RAM for bwa-mem2
    def mem_pipe = 4

    """
    # CRITICAL: Ensure temp directory exists
    mkdir -p ${params.tmp_dir}

    # 1. Convert Fastq to uBAM (Unmapped BAM) -> USES BIG MEMORY
    ...
    # 3. MarkDuplicatesWithMateCigar -> BACK TO BIG MEMORY
...
process MERGE_SINGLE_SAMPLE_GVCFS {
    ...
    script:
    // Nextflow native way to format multiple input files!
    // This securely converts the list of files into "-I file1 -I file2 -I file3"
...
workflow {
    ...
    GENOTYPE_GVCFS(MERGE_SINGLE_SAMPLE_GVCFS.out.merged_gvcf, ref_ch, ref_dict_ch, ref_idx_ch)
